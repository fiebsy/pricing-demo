...
import { Menu } from '@/components/ui/core'
import type { MenuItemType, MenuItemAction, MenuAppearance, AnimationConfig, MenuSide, MenuAlign } from '@/components/ui/core'
...
function transformItemsWithFilterState(
  ...
  return items.map((item) => {
    ...
    if (item.type === 'submenu') {
      let activeCount = 0
      const wrappedSubItems = item.items.map((subItem: MenuItemType) => {
