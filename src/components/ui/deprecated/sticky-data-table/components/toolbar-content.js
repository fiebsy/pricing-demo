...
const column_control_panel_1 = require("./column-control-panel");
const export_toolbar_1 = require("./export-toolbar");
// ============================================================================
// COMPONENT
// ============================================================================
/**
 ...
 */
function ToolbarContent({ isIntegrated, toolbarLayout, leftToolbar, rightToolbar, showCount, totalCount, countLabel, showExportButton, exportAll, exportSelected, exportToolbar, selectionState, showColumnControl, columnGroups, columnsWithSelection, visibleColumnKeys, toggleColumn, resetColumns, columnLabelsWithCheckbox, }) {
    ...
    const renderDefaultCount = (value, label) => (<>
      <span className="text-secondary">{value?.toLocaleString()}</span> {label}
    </>);
    // Count element factory - left side
    const renderLeftCount = () => showLeftCount && (<div className={`${!leftCountRenderer ? 'text-tertiary text-xs font-medium' : ''} ${toolbarLayout.debug ? 'bg-blue-300/50 p-0.5' : ''}`} style={{
    ...
    const renderRightCount = () => showRightCount && (<div className={`${!rightCountRenderer ? 'text-tertiary text-xs font-medium' : ''} ${toolbarLayout.debug ? 'bg-green-300/50 p-0.5' : ''}`} style={{
    ...
    return (<div className={`flex items-end gap-4 ${leftToolbar || showLeftCount ? 'justify-between' : 'justify-end'} ${isIntegrated ? 'w-full' : ''} ${toolbarLayout.debug ? 'bg-purple-500/10 p-1' : ''}`}>
      {/* Left Column: leftToolbar + optional count (above/below/inline) */}
      {(leftToolbar || showLeftCount) && (<div className={`${leftColumnClass} ${toolbarLayout.debug ? 'bg-blue-500/20 p-1 outline outline-1 outline-blue-500' : ''}`}>
          {/* Count above toolbar */}
          {stackPosition === 'above' && renderLeftCount()}
          {leftToolbar && (<div className={`flex ${toolbarMinHeight} items-center ${toolbarLayout.debug ? 'bg-blue-500/30 p-0.5' : ''}`}>
              {leftToolbar}
            </div>)}
      ...
      <div className={`${rightColumnClass} ${toolbarLayout.debug ? 'bg-green-500/20 p-1 outline outline-1 outline-green-500' : ''}`}>
        ...
        <div className={`flex ${toolbarMinHeight} items-center justify-end gap-2 ${toolbarLayout.debug ? 'bg-green-500/30 p-0.5' : ''}`}>
          {rightToolbar && (<div className="flex items-center">
              {rightToolbar}
            </div>)}
          {showExportButton && (<export_toolbar_1.ExportToolbar exportAll={exportAll} exportSelected={exportSelected} selectionState={selectionState} customToolbar={exportToolbar}/>)}
          {showColumnControl && (<column_control_panel_1.ColumnControlPanel columnGroups={columnGroups} allColumns={columnsWithSelection} visibleColumnKeys={visibleColumnKeys} onToggleColumn={toggleColumn} onResetColumns={resetColumns} columnLabels={columnLabelsWithCheckbox}/>)}
        </div>
...
//# sourceMappingURL=toolbar-content.js.map