...
getShiftDirection, getInlineOffset, getShiftAmount, isInlineDragMode = false, draggedColumnKey, }) {
    ...
    return (<a href="#" className={`grid w-fit min-w-full ${rowBorderClass} hover:bg-secondary no-underline last:border-b-0`} style={{
      ...
      {allColumns.map((col, colIndex) => {
            ...
            const cellContent = isCheckboxColumn && selectionState ? (<div data-checkbox-cell="true">
            <checkbox_1.Checkbox isSelected={selectionState.isRowSelected(rowId)} onChange={() => selectionState.toggleRowSelection(rowId)} onClick={(e) => e.stopPropagation()} aria-label="Select row" size="sm"/>
