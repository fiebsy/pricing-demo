 ...
 */
function useDimensions(contentRef, options = {}) {
    const { onDimensionsChange, fadeIn = true, initialDimensions } = options;
    const resizeTimeoutRef = (0, react_1.useRef)(null);
    const [dimensions, setDimensions] = (0, react_1.useState)(() => initialDimensions ?? { width: 0, height: 0 });
    const [hasMeasured, setHasMeasured] = (0, react_1.useState)(() => (!fadeIn ? true : !!initialDimensions));
    // Use refs to avoid re-running effects
    const dimensionsRef = (0, react_1.useRef)(dimensions);
    const hasMeasuredRef = (0, react_1.useRef)(hasMeasured);
    const onDimensionsChangeRef = (0, react_1.useRef)(onDimensionsChange);
    (0, react_1.useEffect)(() => {
