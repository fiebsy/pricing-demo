 ...
 */
export const SkwircleShadow: React.FC<SkwircleShadowProps> = ({
  dimensions,
  borderWidth,
  shadowConfig,
  path,
}) => {
  if (!path || shadowConfig.opacity <= 0) return null
  ...
  return (
    <svg
      ...
      viewBox={`${-blur} ${-blur} ${expandedWidth} ${expandedHeight}`}
      style={{
        position: 'absolute',
        top: offsetY - blur,
        left: offsetX - blur,
