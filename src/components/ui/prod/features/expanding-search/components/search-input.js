...
const SearchInput = ({ inputRef, isExpanded, value, placeholder, collapsedWidth, contentOpacity, contentTransition, onChange, onKeyDown, children, }) => {
    return (<div className="absolute top-0 bottom-0 right-0 flex items-center gap-2 pr-3" style={{
      ...
      <input_1.Input ref={inputRef} type="search" value={value} onChange={onChange} onKeyDown={onKeyDown} placeholder={placeholder} className={(0, utils_1.cn)('flex-1 min-w-0 bg-transparent border-none outline-none', 'text-primary text-sm', 'placeholder:text-tertiary')} aria-label="Search" tabIndex={isExpanded ? 0 : -1}/>
      {children}
    </div>);
};
