...
import { sizePresets } from '../../config'
import type {
...
const AccordionAnimatedLineContext = createContext<AccordionAnimatedLineContextValue | null>(null)

const useAccordionAnimatedLineContext = () => {
  const context = useContext(AccordionAnimatedLineContext)
  if (!context) {
...
const AccordionAnimatedLineItem: React.FC<AccordionAnimatedLineItemProps & { index?: number }> = ({
  value,
  children,
  href,
  onClick,
...
const AccordionAnimatedLineRoot: React.FC<AccordionAnimatedLineProps> = ({
  label,
  icon,
  children,
  ...
  // Merge line config
  const mergedLineConfig = useMemo(
    () => ({
      ...defaultLineConfig,
      ...lineConfig,
    }),
  ...
  const lineGeometry = useMemo(
    () =>
      calculateLineGeometry({
        itemHeight: itemHeightBySize[size],
        itemGap: itemGapBySize[size],
  ...
  // Context value
  const contextValue = useMemo<AccordionAnimatedLineContextValue>(
    () => ({
      isExpanded,
      size,
      lineGeometry,
      config: {
        ...
        paddingX: 8,
        borderRadius: 6,
        animationDistance: animationDistanceBySize[size],
  ...
  const indexedChildren = React.Children.map(children, (child, index) => {
    if (React.isValidElement<AccordionAnimatedLineItemProps & { index?: number }>(child)) {
  ...
  const maxHeight = useMemo(() => {
    const itemsHeight = childCount * itemHeightBySize[size]
    const gapsHeight = Math.max(0, childCount - 1) * itemGapBySize[size]
    const padding = mergedLineConfig.topPadding
    return itemsHeight + gapsHeight + padding + 20
  }, [childCount, size, mergedLineConfig])

  return (
    <AccordionAnimatedLineContext.Provider value={contextValue}>
          ...
          {mergedTriggerConfig.showChevron && (
            ...
            >
              <HugeIcon
                ...
                style={{ transitionDuration: `${mergedAnimationConfig.chevronDuration}ms` }}
              />
              <HugeIcon
...
export const AccordionAnimatedLine = Object.assign(AccordionAnimatedLineRoot, {
