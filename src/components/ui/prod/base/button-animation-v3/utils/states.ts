/**
 * ButtonAnimation V3 - State Utilities
 *
 * Minimal state management for clear animation targeting.
 *
...
/**
 * Determine the animation state for a button based on context.
 * This is the core logic that fixes the child-to-parent animation issue.
 */
export function determineButtonState(context: StateContext): AnimationState {
  const {
    itemId,
    level,
    hasChildren,
  ...
  if (isActive) {
    ...
    if (hasActiveChild) {
      ...
      if (wasChild && level > 0) {
        // This is the key fix: detect promotion state
  ...
  switch (state) {
    ...
    case AnimationState.ANCHORED:
      return {
        opacity: 0.6,
        scale: 1,
        zIndex: 10, // Will be adjusted by depth
        interactive: false,
      }
