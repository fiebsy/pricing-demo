...
exports.DragCursor = exports.DragClone = void 0;
/**
...
function DragClone({ dragState }) {
    ...
    return (0, react_dom_1.createPortal)(<div className="pointer-events-none fixed z-[9999]" style={{
            left: cloneLeft,
      ...
      <div className="relative flex flex-col overflow-hidden opacity-80" style={{
        ...
        <div className="flex flex-col overflow-hidden">
          ...
          <div className={`flex items-center bg-primary/95 px-3 text-xs font-medium text-tertiary backdrop-blur-sm ${alignClass}`} style={{ height: dragState.dragHeight }}>
            {dragState.dragLabel}
          </div>
          {/* Skeleton rows with gradient fade effect */}
          <div className="relative bg-primary/40">
            {/* Row 1 - most visible */}
            <div className={`flex items-center border-t border-primary/60 px-3 ${alignClass}`} style={{ height: 46, opacity: 0.85 }}>
              <div className="h-3.5 w-3/4 rounded-sm bg-tertiary"/>
            </div>
            {/* Row 2 - fading */}
            <div className={`flex items-center border-t border-primary/40 px-3 ${alignClass}`} style={{ height: 46, opacity: 0.5 }}>
              <div className="h-3.5 w-1/2 rounded-sm bg-tertiary"/>
            </div>
            {/* Row 3 - faded out */}
            <div className={`flex items-center border-t border-primary/20 px-3 ${alignClass}`} style={{ height: 46, opacity: 0.2 }}>
...
exports.DragClone = DragClone;
/**
 * Global grabbing cursor during drag - rendered via portal to document.head
 */
function DragCursor({ isDragging }) {
    if (!isDragging || typeof document === 'undefined') {
        return null;
    }
    return (0, react_dom_1.createPortal)(<style>{`* { cursor: grabbing !important; }`}</style>, document.head);
}
exports.DragCursor = DragCursor;
//# sourceMappingURL=drag-clone.js.map