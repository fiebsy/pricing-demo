...
const FilterPill = ({ filter, category, onRemove }) => {
    return (<div className={(0, utils_1.cn)('group inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5', 'bg-primary border border-primary', 'text-sm font-medium text-secondary', 'transition-all duration-150 ease-out')}>
      ...
      <button onClick={onRemove} className={(0, utils_1.cn)('ml-0.5 -mr-0.5 rounded p-0.5', 'text-quaternary hover:text-secondary hover:bg-secondary', 'transition-colors duration-100', 'outline-hidden focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-brand')}>
...
const FilterDropdown = ({ categories, filterState, onFilterAdd, onFilterRemove, onFiltersClear, className, showClearAll = true, }) => {
    ...
    return (<div className={(0, utils_1.cn)('flex flex-wrap items-center gap-2', className)}>
      {/* Active Filter Pills */}
      ...
      <menu_1.Menu trigger={<button className={(0, utils_1.cn)('inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5', 'bg-primary border border-primary', 'text-sm font-medium text-tertiary', 'hover:bg-secondary hover:text-secondary transition-all duration-150', 'outline-hidden focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand')}>
            <icon_1.HugeIcon icon={Add01Icon_1.default} size={16} strokeWidth={2}/>
            <span>Add a filter</span>
          </button>} items={menuItems} header={searchHeader} width={280} side="bottom" align="start" onOpenChange={handleOpenChange} animation={{
            duration: 300,
      ...
      {/* Clear All Button */}
      {showClearAll && filterState.hasFilters && (<button onClick={onFiltersClear} className={(0, utils_1.cn)('text-sm font-medium text-tertiary hover:text-secondary transition-colors', 'outline-hidden focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand')}>
          Clear all
        </button>)}
    </div>);
};
