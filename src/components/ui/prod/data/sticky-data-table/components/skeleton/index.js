...
exports.createSkeletonConfigFromTableProps = exports.LoadMoreSkeleton = exports.TableSkeleton = void 0;
/**
...
// Internal components
const skeleton_header_1 = require("./skeleton-header");
const skeleton_body_1 = require("./skeleton-body");
const toolbar_skeleton_1 = require("./toolbar-skeleton");
 ...
 * ```
 */
const TableSkeleton = ({ columns, skeletonRowCount = 10, showToolbar = false, showLeftToolbar = false, borderRadius = config_1.TABLE_CONFIG.DEFAULT_BORDER_RADIUS, className, toolbarConfig, rowHeight, headerHeight, dimensionConfig, 
// New sticky synchronization props
enableSelection = false, defaultVisibleColumns, simulateStickyState = 'auto', borderConfig: borderConfigOverrides, backgroundConfig: backgroundConfigOverrides, toolbarLayout, 
// Cell configuration
headerCellConfig, bodyCellConfig, }) => {
    // Track actual overflow state (measured from DOM)
    const [hasOverflow, setHasOverflow] = (0, react_1.useState)(null);
    // Process columns with unified logic
    const processed = (0, react_1.useMemo)(() => {
        // Create visible column keys set
        const visibleColumnKeys = (0, utils_1.createVisibleColumnKeys)(columns, defaultVisibleColumns);
        // Determine if we should simulate scrollable state
        let simulateScrollable;
    ...
    const effectiveConfig = (0, react_1.useMemo)(() => {
        if (dimensionConfig) {
            return dimensionConfig;
        }
        if (toolbarConfig) {
            return (0, config_1.createSkeletonDimensionConfig)({
    ...
    const effectiveRowHeight = effectiveConfig.rowHeight ?? config_1.TABLE_CONFIG.ROW_HEIGHT;
    const totalHeight = (0, config_1.calculateSkeletonHeight)(effectiveConfig, skeletonRowCount, toolbarLayout);
    ...
    return (<div className={className} style={{ height: totalHeight }}>
      {/* Toolbar Skeleton - Only render when NOT integrated */}
      {toolbarSettings.showToolbar && !isToolbarIntegrated && (<toolbar_skeleton_1.ToolbarSkeleton showFilter={toolbarSettings.showLeftToolbar} showSearch={toolbarSettings.showRightToolbar} showExport={toolbarSettings.showExportButton} showColumnControl={toolbarSettings.showColumnControl} showCount={toolbarSettings.showCount} toolbarLayout={toolbarLayout}/>)}

      {/* Sticky Header Skeleton (includes integrated toolbar when applicable) */}
      <skeleton_header_1.SkeletonHeaderSticky processed={processed} borderRadius={effectiveConfig.borderRadius ?? borderRadius} toolbarLayout={toolbarLayout} toolbarConfig={toolbarSettings} headerHeight={headerHeight} headerCellConfig={headerCellConfig}/>

      {/* Body Skeleton */}
      <skeleton_body_1.SkeletonBodySticky processed={processed} rowCount={skeletonRowCount} rowHeight={effectiveRowHeight} borderRadius={effectiveConfig.borderRadius ?? borderRadius} onOverflowDetected={simulateStickyState === 'auto' ? setHasOverflow : undefined} bodyCellConfig={bodyCellConfig}/>
    </div>);
};
exports.TableSkeleton = TableSkeleton;
const LoadMoreSkeleton = ({ columns, rowCount, borderRadius = config_1.TABLE_CONFIG.DEFAULT_BORDER_RADIUS, asRowsOnly = false, rowHeight = config_1.TABLE_CONFIG.ROW_HEIGHT, enableSelection = false, defaultVisibleColumns, borderConfig: borderConfigOverrides, backgroundConfig: backgroundConfigOverrides, stickyState, bodyCellConfig, skipHeaderOverlap = false, }) => {
    // Process columns with unified logic
    const processed = (0, react_1.useMemo)(() => {
        const visibleColumnKeys = (0, utils_1.createVisibleColumnKeys)(columns, defaultVisibleColumns);
    ...
    const rows = Array.from({ length: rowCount }).map((_, i) => (<skeleton_body_1.SkeletonRowSticky key={`load-more-${i}`} processed={processed} index={i} isLast={i === rowCount - 1 && !asRowsOnly} rowHeight={rowHeight} bodyCellConfig={bodyCellConfig}/>));
    if (asRowsOnly) {
        return <>{rows}</>;
    }
    const outerBorderClasses = (0, utils_2.getBodyOuterBorders)(processed.borderConfig);
    const outerBorderStyles = (0, utils_2.getBodyOuterBorderStyles)(processed.borderConfig);
    return (<div className={`overflow-x-auto ${outerBorderClasses} ${processed.backgroundConfig.bodyContainer}`} style={{
            borderBottomLeftRadius: `${borderRadius}px`,
            borderBottomRightRadius: `${borderRadius}px`,
            // Only apply negative margin when connecting to skeleton header (not real header)
            marginTop: skipHeaderOverlap ? 0 : -borderRadius,
            position: 'relative',
            zIndex: 0,
            scrollbarWidth: 'none',
...
exports.LoadMoreSkeleton = LoadMoreSkeleton;
// ============================================================================
...
function createSkeletonConfigFromTableProps(tableProps) {
    ...
    return {
        ...
        borderRadius: tableProps.borderRadius ?? config_1.TABLE_CONFIG.DEFAULT_BORDER_RADIUS,
...
exports.createSkeletonConfigFromTableProps = createSkeletonConfigFromTableProps;
