...
// Import base panel builders for inherited sections
import { buildQuickFixPanelConfig } from '../../quick-fix-interactions/panels/panel-config'
...
function getInheritedSections(config: QuickFixModalConfig): Section[] {
  // Build the base panel config
  const basePanel = buildQuickFixPanelConfig(config, [], null)

  // Filter out Solution and Preview sections (we have our own)
  return basePanel.sections.filter(
    (s) => !['solution', 'preview'].includes(s.id)
  )
}
