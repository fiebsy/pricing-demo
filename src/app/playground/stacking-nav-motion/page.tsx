/**
 * StackingNav Motion Playground
 *
 * Uses Motion's layout="position" for anchored item repositioning.
 *
 * @module playground/stacking-nav-motion
 */
...
export default function StackingNavMotionPlayground() {
            ...
            {Array.from({ length: 12 }).map((_, rowIndex) => (
              <div key={rowIndex} className="flex gap-4">
                {Array.from({ length: 4 }).map((_, colIndex) => (
                  <div
                    key={colIndex}
                    className="flex-1 h-32 rounded-2xl bg-tertiary/50 border border-primary/10"
                  />
                ))}
              </div>
            ))}
