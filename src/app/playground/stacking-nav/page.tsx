...
export default function StackingNavPlayground() {
  ...
  const animationConfig: Partial<AnimationConfig> = useMemo(
    () => {
      ...
      return {
        ...
        promotionDelay: (config.promotionDelay / 1000) / scale,
        promotionOpacity: config.promotionOpacity,
        promotionVariant: config.promotionVariant,
        demotionDuration: (config.demotionDuration / 1000) / scale,
        demotionScale: config.demotionScale,
        demotionDelay: (config.demotionDelay / 1000) / scale,
        demotionOpacity: config.demotionOpacity,
        demotionVariant: config.demotionVariant,
  ...
  const styleConfig: Partial<StyleConfig> = useMemo(
    () => ({
      ...
      clipAnimated: config.clipAnimated,
