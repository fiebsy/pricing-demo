...
const filter_select_chip_motion_1 = require("@/components/ui/prod/base/filter/filter-select-chip-motion");
...
const DEFAULT_CONFIG = {
    ...
    preset: 'snappy',
    usePreset: true,
    transitionType: filter_select_chip_motion_1.DEFAULT_ANIMATION_CONFIG.transitionType,
    easing: filter_select_chip_motion_1.DEFAULT_ANIMATION_CONFIG.easing,
    duration: filter_select_chip_motion_1.DEFAULT_ANIMATION_CONFIG.duration,
    stiffness: filter_select_chip_motion_1.DEFAULT_ANIMATION_CONFIG.stiffness,
    damping: filter_select_chip_motion_1.DEFAULT_ANIMATION_CONFIG.damping,
    exitDuration: filter_select_chip_motion_1.DEFAULT_ANIMATION_CONFIG.exitDuration,
    // Style
    size: filter_select_chip_motion_1.DEFAULT_STYLE_CONFIG.size,
    gap: filter_select_chip_motion_1.DEFAULT_STYLE_CONFIG.gap,
    roundness: filter_select_chip_motion_1.DEFAULT_STYLE_CONFIG.roundness,
    // Demo
    showDebug: false,
    autoAdd: false,
...
// PANEL CONFIGURATION
// =============================================================================
const createAnimationSection = (config) => ({
    id: 'animation',
    title: 'Animation',
    tabLabel: 'Animate',
    subsections: [
        {
            ...
            controls: [
                ...
                },
                {
                    ...
                    type: 'select',
                    value: config.preset,
                    options: filter_select_chip_motion_1.PRESET_OPTIONS,
            ...
            controls: [
                {
                    ...
                    options: filter_select_chip_motion_1.TRANSITION_TYPE_OPTIONS,
                    disabled: config.usePreset,
                },
            ],
        },
            ...
            controls: [
                {
                    ...
                    type: 'select',
                    value: config.easing,
                    options: filter_select_chip_motion_1.EASING_OPTIONS,
                ...
                },
                {
                    ...
                    max: 0.5,
                    step: 0.01,
                    formatLabel: (v) => `${(v * 1000).toFixed(0)}ms`,
                    disabled: config.usePreset || config.transitionType !== 'tween',
                },
            ],
        },
...
const createStyleSection = (config) => ({
    id: 'style',
    title: 'Style',
    tabLabel: 'Style',
    subsections: [
            ...
            controls: [
                {
                    ...
                    type: 'select',
                    value: config.size,
                    options: filter_select_chip_motion_1.SIZE_OPTIONS,
                },
                {
                    id: 'roundness',
                    label: 'Roundness',
                    ...
                    options: filter_select_chip_motion_1.ROUNDNESS_OPTIONS,
                },
                {
                    id: 'gap',
                    label: 'Gap',
                    type: 'select',
                    value: config.gap,
                    options: filter_select_chip_motion_1.GAP_OPTIONS,
                },
            ],
        },
    ],
});
const createDemoSection = (config) => ({
    id: 'demo',
    title: 'Demo',
    tabLabel: 'Demo',
    subsections: [
        ...
        },
        {
            ...
            controls: [
                ...
                },
                {
                    ...
                    max: 5000,
                    step: 250,
                    formatLabel: (v) => `${(v / 1000).toFixed(1)}s`,
...
// MAIN PAGE
// =============================================================================
function FilterChipMotionPlayground() {
    ...
    const animationConfig = (0, react_1.useMemo)(() => {
        if (config.usePreset) {
            return (0, filter_select_chip_motion_1.getPresetConfig)(config.preset);
        }
        return {
    ...
    // Handler: Add filter
    const handleAddFilter = (0, react_1.useCallback)((filterId) => {
    ...
    const handleFilterChange = (0, react_1.useCallback)((filterId, value) => {
        setActiveFilters((prev) => prev.map((f) => (f.id === filterId ? { ...f, value } : f)));
    }, []);
    ...
    const handleAddAll = (0, react_1.useCallback)(() => {
      ...
      <div className="pr-[352px] pb-24 md:pb-0">
        <div className="flex flex-col min-h-[calc(100vh-120px)] p-8">
          ...
          <div className="mb-6 flex flex-wrap items-center gap-2">
            ...
            {availableToAdd.map((filter) => (<button key={filter.id} onClick={() => handleAddFilter(filter.id)} className="px-3 py-1.5 text-xs font-medium rounded-full bg-secondary text-secondary hover:bg-tertiary hover:text-primary transition-colors">
                + {filter.label}
              </button>))}
            {availableToAdd.length === 0 && (<span className="text-xs text-quaternary italic">
                All filters active
              </span>)}
          </div>
          ...
          <div className="bg-secondary rounded-2xl p-6 mb-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={handleClearAll} disabled={activeFilters.length === 0} className="px-3 py-1 text-xs rounded-lg bg-tertiary text-secondary hover:text-primary disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  Clear All
                </button>
                <button onClick={handleAddAll} disabled={availableToAdd.length === 0} className="px-3 py-1 text-xs rounded-lg bg-brand-primary text-primary_on-brand hover:bg-brand-primary_hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  Add All
                </button>
              </div>
            </div>

            {activeFilters.length > 0 ? (<filter_select_chip_motion_1.FilterSelectChipMotion filters={activeFilters} onFilterChange={handleFilterChange} onFilterRemove={handleFilterRemove} animationConfig={animationConfig} styleConfig={styleConfig} showDebug={config.showDebug}/>) : (<div className="flex items-center justify-center h-16 text-quaternary text-sm">
...
exports.default = FilterChipMotionPlayground;
