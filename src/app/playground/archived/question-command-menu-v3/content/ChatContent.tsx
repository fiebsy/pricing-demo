/**
 * Question Command Menu V3 - Chat Content
 *
 * Displays a mini chat interface with AI responses.
 * Features:
 * - Auto-scroll to bottom when new messages arrive
 * - Snap to bottom - messages anchor at bottom of container
...
import type { ChatMessage, ChatConfig, SlotPosition } from '../config/types'
...
export const ChatContent: React.FC<ChatContentProps> = ({
  ...
  const { config, getSlotConfig } = useV3Context()
  ...
  const handleAction = (messageId: string, actionId: string) => {
    ...
    // speak action could be implemented with Web Speech API
  }
  ...
  useEffect(() => {
    ...
    // Check if we should auto-scroll (user at bottom or new message)
