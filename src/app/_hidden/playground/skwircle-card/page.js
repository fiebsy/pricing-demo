...
const path_generator_1 = require("@/components/ui/deprecated/skwircle/utils/path-generator");
const roundness_1 = require("@/components/ui/deprecated/skwircle/config/roundness");
const constants_1 = require("@/components/ui/deprecated/skwircle/config/constants");
...
function ShowcaseCard({ config }) {
    ...
    const depthPreset = config.depthIntensity === 'none'
        ? undefined
        : `depth-${config.depthIntensity}-${config.depthDirection}`;
    // Aspect ratio style
    ...
    const clipPath = (0, react_1.useMemo)(() => {
        if (dimensions.width === 0 || dimensions.height === 0)
            return undefined;
        const roundnessConfig = roundness_1.ROUNDNESS_CONFIGS[config.roundness];
        const innerConfig = {
        ...
        const path = (0, path_generator_1.generateSkwirclePath)(dimensions.width, dimensions.height, innerConfig, 0);
        return `path('${path}')`;
    }, [dimensions.width, dimensions.height, config.roundness, config.borderWidth]);
    // Build card style with dimensions
    const cardStyle = config.fillWidth
        ? { width: '100%' }
    ...
    const getBorderCssVar = (token) => token.replace('border-', 'border-color-');
    ...
    const getCustomBorderGradient = () => {
        if (config.borderGradient === 'none' || config.borderColor === 'none')
            return undefined;
        const preset = constants_1.GRADIENT_BORDER_PRESETS[config.borderGradient];
        if (!preset)
            return undefined;
    ...
    const getPreviewBackgroundClass = () => {
        if (config.previewBackground === 'white')
            return 'bg-white';
    ...
    const getPreviewGradientStyle = () => {
        if (config.previewGradientSize === 'none')
            return undefined;
        const colorMap = {
        ...
        return `linear-gradient(238deg, ${orderedStops.map((opacity, i) => `color-mix(in srgb, ${color} ${opacity}%, transparent) ${i * 25}%`).join(', ')})`;
    };
    return (<skwircle_1.Skwircle.Card elevation={config.elevation} roundness={config.roundness} borderWidth={config.borderWidth} borderColor={borderColorWithOpacity} borderGradient={config.borderGradient !== 'none' ? 'custom' : 'none'} customBorderGradient={getCustomBorderGradient()} ring={config.ringEnabled} ringColor={ringColorWithOpacity} ringWidth={config.ringWidth} ringOpacity={config.ringOpacity} backgroundGradient={depthPreset} intent="default" fillMode style={cardStyle}>
      {/* Content wrapper with squircle clip path */}
      <div ref={contentRef} className="flex flex-col flex-1 w-full" style={{ clipPath }}>
        {/* Preview Area */}
        <div className={`flex flex-1 items-center justify-center w-full ${getPreviewBackgroundClass()}`} style={{
            ...aspectStyle,
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="flex items-center gap-1 rounded-lg bg-gray-900 px-2 py-1.5">
              {['💬', '↗', '👁', '👁', '⬆', '☰'].map((icon, i) => (<span key={i} className="flex h-7 w-7 items-center justify-center text-sm text-gray-400">
                  {icon}
                </span>))}
            </div>
          </div>
        ...
        {/* Footer */}
        <div className={`${footerColors.bg} w-full`} style={{
...
exports.default = SkwircleCardPage;
