...
export default function ComponentPlaygroundPage() {
  ...
  const panelConfig = useMemo<UnifiedControlPanelConfig>(() => ({
    ...
    position: {
      top: '16px',      // Top of viewport (nav-clearance handles nav offset)
  ...
  return (
    <div className="min-h-screen">
      {/* Breadcrumbs - nav-clearance provides padding for FloatingNav */}
...
### 2. Panel Position & Navigation
- Use `top: '16px'` for panel position (nav-clearance handles nav offset)
- Use `nav-clearance px-6` wrapper for Breadcrumbs navigation
- Use `pr-[352px] pb-24 md:pb-0` on preview area (panel width + mobile nav padding)
