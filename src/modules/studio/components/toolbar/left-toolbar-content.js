...
const FILTER_OPTIONS_BY_CATEGORY = {
    Status: [
        ...
        { id: 'status-invited', label: 'Invited' },
        { id: 'status-revoked', label: 'Revoked' },
    ],
    'Last Interacted': [
    ...
    Messages: [
        { id: 'messages-0-10', label: '0-10' },
        { id: 'messages-11-50', label: '11-50' },
        { id: 'messages-51-100', label: '51-100' },
        { id: 'messages-101-500', label: '101-500' },
        { id: 'messages-500+', label: '500+' },
        { id: 'messages-51+', label: '51+ (Most Engaged)' },
    ],
    Tags: [
        { id: 'tag-vip', label: 'VIP' },
        { id: 'tag-lead', label: 'Lead-Coaching' },
        { id: 'tag-client', label: 'Client' },
        { id: 'tag-cohort', label: 'Cohort-Jan-2025' },
        { id: 'tag-inactive', label: 'Inactive' },
    ],
};
// =============================================================================
...
const LeftToolbarContent = ({ activeFilters, onFilterSelect, onFilterChange, onFilterRemove, onFiltersClear, className, }) => {
    ...
    const filterChipData = activeFilters.map((filter) => ({
        id: filter.category.toLowerCase().replace(' ', '-'), // Stable key based on category
        label: filter.category, // Category name shown in dropdown header
        icon: filter_config_1.FILTER_ICONS[filter.category],
        value: filter.id, // Currently selected option ID
    ...
    const handleFilterChange = (categoryId, newValue) => {
        ...
        const currentFilter = activeFilters.find((f) => f.category.toLowerCase().replace(' ', '-') === categoryId);
        if (currentFilter) {
    ...
    return (<div className={(0, utils_1.cn)('flex flex-wrap items-center gap-3', className)}>
      {/* Filter Menu */}
      <filter_1.FilterMenu trigger={<filter_1.FilterTrigger isOpen={isMenuOpen} label="Filter" size="md" rounded="full" variant="default"/>} items={filter_config_1.AUDIENCE_FILTER_ITEMS} onFilterSelect={onFilterSelect} activeFilterIds={activeFilters.map((f) => f.id)} onOpenChange={setIsMenuOpen} align="start" side="bottom" width={240}/>
