...
import {
  type PartnerRiskItem,
  type MockDataMetrics,
  RiskCategory,
  RiskTier,
  PaymentPlanType,
  ContractStatus,
...
const COLLECTIONS_STATUSES = [
  ContractStatus.FailedPayment,
  ContractStatus.NsfReturned,
  ContractStatus.PaymentPending,
]

const CLAWBACK_STATUSES_DEFAULTED = [
  ContractStatus.DefaultedPendingClawback,
  ContractStatus.DefaultedSettled,
]

const CLAWBACK_STATUSES_CANCELED = [
  ContractStatus.CanceledPendingClawback,
  ContractStatus.CanceledSettled,
]

const CLAWBACK_STATUSES_CHARGEBACK = [
  ContractStatus.ChargebackPendingClawback,
  ContractStatus.ChargebackSettled,
]

const STATUS_DESCRIPTIONS: Record<string, { description: string; tooltip: string }> = {
  [ContractStatus.FailedPayment]: {
 ...
 */
const generateEmail = (name: string, seed: number): string => {
 ...
 */
const generateRelativeTime = (date: Date): string => {
  const now = new Date()
 ...
 */
const generateStatusChangedAt = (daysInCollections: number): Date => {
  const now = new Date()
 ...
 */
const generateCollectionsItem = (index: number, seed: number): PartnerRiskItem => {
  ...
  const planType = seededRandom(seed + 8) > 0.5 ? PaymentPlanType.Servicing : PaymentPlanType.Funding
  ...
  return {
    ...
    riskCategory: RiskCategory.Collections,
    riskTier,
    status,
 ...
 */
const generateActiveClawbackItem = (index: number, seed: number): PartnerRiskItem => {
  const customerName = generateCustomerName(seed)
  ...
  if (outcomeRoll < 0.6) {
    status = randomFromArray(CLAWBACK_STATUSES_DEFAULTED.filter(s => s.includes('Pending')), seed + 14) || ContractStatus.DefaultedPendingClawback
    outcomeType = 'Default'
  } else if (outcomeRoll < 0.85) {
    status = randomFromArray(CLAWBACK_STATUSES_CANCELED.filter(s => s.includes('Pending')), seed + 14) || ContractStatus.CanceledPendingClawback
    outcomeType = 'Canceled'
  } else {
    status = randomFromArray(CLAWBACK_STATUSES_CHARGEBACK.filter(s => s.includes('Pending')), seed + 14) || ContractStatus.ChargebackPendingClawback
  ...
  return {
    ...
    clawbackAmount: null,
    shortfallAmount: null,
    riskCategory: RiskCategory.ActiveClawback,
    riskTier: null,
    status,
    previousStatus: ContractStatus.FailedPayment,
    statusChangedAt,
    statusTransitionDescription: statusInfo.description,
    statusTransitionTooltip: statusInfo.tooltip,
    statusChangedRelative: generateRelativeTime(statusChangedAt),
 ...
 */
const generateSettledClawbackItem = (index: number, seed: number): PartnerRiskItem => {
  const customerName = generateCustomerName(seed)
  ...
  return {
    ...
    clawbackAmount,
    shortfallAmount,
    riskCategory: RiskCategory.SettledClawback,
    riskTier: null,
    status,
    previousStatus: ContractStatus.ClawbackInProgress,
    statusChangedAt,
    statusTransitionDescription: statusInfo.description,
    statusTransitionTooltip: statusInfo.tooltip,
    statusChangedRelative: generateRelativeTime(statusChangedAt),
 ...
 */
export const generateMockData = (): PartnerRiskItem[] => {
  const items: PartnerRiskItem[] = []
 ...
 */
export const calculateMockMetrics = (items: PartnerRiskItem[]): MockDataMetrics => {
  const collectionsItems = items.filter(i => i.riskCategory === RiskCategory.Collections)
  const activeClawbackItems = items.filter(i => i.riskCategory === RiskCategory.ActiveClawback)
  const settledClawbackItems = items.filter(i => i.riskCategory === RiskCategory.SettledClawback)
  ...
  const defaultedOrdersCount = items.filter(i =>
    i.status === ContractStatus.DefaultedPendingClawback ||
    i.status === ContractStatus.DefaultedSettled
  ).length
...
let _mockData: PartnerRiskItem[] | null = null
let _mockMetrics: MockDataMetrics | null = null

/**
 * Lazily generated mock data - only computed on first access
 */
export const MOCK_DATA: PartnerRiskItem[] = new Proxy([] as PartnerRiskItem[], {
  get(target, prop) {
    if (_mockData === null) {
 ...
 */
export const MOCK_METRICS: MockDataMetrics = new Proxy({} as MockDataMetrics, {
  get(target, prop) {
    if (_mockMetrics === null) {
      if (_mockData === null) {
        _mockData = generateMockData()
      }
      _mockMetrics = calculateMockMetrics(_mockData)
    }
